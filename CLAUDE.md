...
- **Test a `say` on the SAME engine the target context uses, or the result is worthless.** The lab first sent `quality: 'fast'` for everything, so its findings were measured on `eleven_turbo_v2_5` while the phrasebook, « Je débute » and quiz words all play through `quality: 'high'` → `eleven_multilingual_v2`. The two pronounce differently: a fatha that fixed `سَلام` on turbo produced « salalam » on multilingual. The lab now has an engine selector defaulting to *Qualité*. Only conversation audio should be tested on *Rapide*.
...
- **Four words resist every spelling, and that is the argument for recorded audio.** Measured over four lab rounds on the Quality engine: `أصلي` is said « sli », `كيفاش` is said « kifech » in all three places it appears, `تهلا` never lands on the right vowel, and `سلام` alone works in a sentence but not isolated. Roughly one problem word in four cannot be fixed by any spelling — arabic, diacritics, plene, latin, joined or split. For those, the only remaining fix is a native recording dropped into the storage slot the synthetic file occupies.
- **Two mechanical rules did emerge, and they explain most cases.** A ة followed by another word reads as « t » (arabic construct state) — replace it with ا; isolated or sentence-final it is already silent, which is why `عشرة`, `خمسة` and `مية` were never wrong despite ending in ة. And **a space can be the whole problem**: `بلا جميل` reads « bila jamil » separated, but correctly when joined as `بلاجميل`, because the engine stops recognising بلا as its own word.
